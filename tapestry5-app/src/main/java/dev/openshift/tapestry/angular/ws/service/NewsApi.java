...
import dev.openshift.tapestry.angular.entity.News;
...
public class NewsApi {
	...
	private News[] getNews() throws JsonParseException, JsonMappingException, IOException{
		//Credentials
		if (this.news == null){
			File jsonNews= new File("src/main/webapp/data/news.json");
			if (!jsonNews.exists()){
	...
	private void addNews(News newNews) throws JsonParseException, JsonMappingException, IOException{
		...
		for (int i= 0; i< theNews.length; i++){
			News currentNews= theNews[i];
			updatedNews[i]= currentNews;
			higherId= Math.max(higherId, currentNews.getId());
		}
